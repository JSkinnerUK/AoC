...
void pop(char *stack, int num, char *out) {
    int length = strlen(stack);
    memcpy(out, &stack[length - num], num);
    stack[length - num] = '\0';
    out[num] = '\0';
}

void push(char *chrs, int chrs_count, char *stack) {
    int length = strlen(stack);
    for(int i = 0; i < chrs_count; i++) {
        stack[length + i] = chrs[i];
    }
    stack[length+chrs_count] = '\0';
}
...
int main(int argc, char *argv[])
{
    ...
    if (fptr != NULL)
    {
        ...
        while ((count = fscanf(fptr, "move %d from %d to %d", &amount, &from, &to)) != EOF) {
            if (count == 3) {
                char popped[amount];
                pop(stacks[from -1], amount, popped);
                push(popped, amount, stacks[to - 1]);
            } else {
